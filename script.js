...
async function initApp() {
    ...
    // Pobieranie planów z plików .tsv (Zawsze świeże z serwera)
    ...
    // Ustalanie domyślnego planu jeśli nic nie jest wybrane
...
// Funkcja kopiowania planu do edycji
...
function saveNotes() { 
    ...
    if (p && !p.isRemote) { 
        p.notes = document.getElementById("planNotes").value; 
        save(); 
    } 
}
        ...
        let rowsHtml = day.rows.map((r, ri) => {
            ...
            return `
                <tr class="${dayChecks[ri] ? 'done' : ''} ${isMoved ? 'moved-row' : ''}">
                    ...
                    <td class="ex-name"><input type="text" value="${r[0]||''}" ${isRemote ? 'readonly' : ''} oninput="updRow(${di},${ri},0,this.value)" placeholder="..."></td>
            ...
            <table>
                <thead>
                    <tr>
                        ...
                        <th style="text-align:left">Ćwiczenie</th>
                        <th class="col-s">S</th>
                        <th class="col-p">P</th>
                        <th class="col-kg">kg</th>
                        ${!isRemote ? '<th class="edit-ui">Opcje</th>' : ''}
                    </tr>
                </thead>
...
function resetWeek() { if(confirm("Zresetować postępy w tym planie?")) { checks[currentPlan] = {}; save(); render(); } }
function newPlan() {
...
function deletePlan() { 
    if(plans[currentPlan] && confirm(`Usunąć Twój plan: ${currentPlan}?`)) { 
...
function applyImport() {
    ...
    if(data.days.length > 0) {
        const n = prompt("Nazwa dla importowanego planu:", "Zaimportowany");
